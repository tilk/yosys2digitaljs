...
const isNodeEnvoronment = typeof process !== "undefined" &&
    process.versions != null &&
    process.versions.node != null;

const assert = isNodeEnvoronment
    ? require('assert')
    : assert_fallback
...
export function prepare_yosys_script(filenames: string[], options: Options): string {
    ...
    const readFilesScript = filenames
        ...
        .map(cmd => isNodeEnvoronment ? shell_escape_contents(cmd) : cmd);
...
export function prepare_verilator_args(filenames: string[]): string[] {
    const processed_filenames = isNodeEnvoronment ? filenames.map(shell_escape) : filenames;
